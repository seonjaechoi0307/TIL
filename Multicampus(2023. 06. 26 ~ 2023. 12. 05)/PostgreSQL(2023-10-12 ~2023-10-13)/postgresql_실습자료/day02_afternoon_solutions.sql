...
SELECT 
	...
	, cohort_retained
	, cohort_retained / FIRST_VALUE(cohort_retained) OVER (PARTITION BY first_year ORDER BY period)::DECIMAL AS pct_retained
FROM
(
 SELECT 
	date_part('year',a.first_term) AS first_year
...
SELECT 
	...
	, cohort_retained
	, cohort_retained * 1.0 / FIRST_VALUE(cohort_retained) OVER (PARTITION BY first_century ORDER BY period)::DECIMAL AS pct_retained
FROM
(
 SELECT 
...
SELECT 
	...
	, cohort_retained
	, cohort_retained * 1.0 / FIRST_VALUE(cohort_retained) OVER (PARTITION BY first_state ORDER BY period)::DECIMAL AS pct_retained
FROM
(
 SELECT 
		a.first_state
...
SELECT
	...
	, COUNT(DISTINCT CASE WHEN tenure >= 10 THEN id_bioguide END) * 1.0 / COUNT(DISTINCT id_bioguide) AS pct_survived_10
FROM (
	SELECT 
		id_bioguide
		, date_part('century', MIN(term_start)) AS first_century
...
SELECT 
	...
	, COUNT(DISTINCT CASE WHEN total_terms >= 5 THEN id_bioguide END) / COUNT(DISTINCT id_bioguide)::DECIMAL AS pct_survived_5_terms
FROM
(
 	SELECT 
		id_bioguide
...
SELECT 
	...
	, COUNT(DISTINCT CASE WHEN a.total_terms >= b.terms THEN id_bioguide END) / COUNT(DISTINCT id_bioguide)::DECIMAL AS pct_survived
FROM
(
	SELECT 
		id_bioguide
